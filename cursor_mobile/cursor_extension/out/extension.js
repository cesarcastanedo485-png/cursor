...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('mordecai.runtimeStatus', async () => {
        try {
            const status = await tasksProvider.getRuntimeStatus();
            const tunnel = status?.state?.tunnel;
            vscode.window.showInformationMessage(`Tunnel running: ${tunnel?.running ? 'yes' : 'no'}${tunnel?.publicUrl ? ` | ${tunnel.publicUrl}` : ''}`);
        }
        catch (e) {
            vscode.window.showErrorMessage(`Runtime status failed: ${e}`);
        }
    }));
    context.subscriptions.push(vscode.commands.registerCommand('mordecai.runtimeStartTunnel', async () => {
        try {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('mordecai.runtimeStopTunnel', async () => {
        try {
