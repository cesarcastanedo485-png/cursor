...
if ($Build) {
    ...
    try {
        ...
        if (Test-Path $keyProps) {
            # Same upload key as CI → Android can upgrade over the existing install without uninstall.
            & $flutter build apk --release -- -PrequireReleaseSigning=true
        } else {
            Write-Host "Warning: android\key.properties missing — release uses DEBUG signing; you cannot upgrade over a store/Drive release-signed install. See docs\SECRETS.md" -ForegroundColor Yellow
