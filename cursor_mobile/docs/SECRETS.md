...
If `key.properties` is missing, release builds fall back to the **debug** keystore (local testing only). **Do not** sideload a debug APK over an install that was signed with your upload key — Android will block the update and require uninstall first.

**GitHub Actions** (`.github/workflows/apk_to_drive.yml`) passes `-PrequireReleaseSigning=true`, so CI **fails** if signing secrets are missing instead of shipping a debug-signed “release” APK.
